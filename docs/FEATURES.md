...
| Untracked source accounts | 🔵 built | `untracked:<key>` in `mapping.json` (value `null`) says a source account is deliberately outside the budget. Its rows are set aside before the reconciliation floor, before transfer pairing and before any FX fetch, and counted on the run line. Wise holds a balance per currency while the budget holds one Wise account, so without this a `wise-aud` row was FX-converted into it and booked as an SGD debit that never happened |
