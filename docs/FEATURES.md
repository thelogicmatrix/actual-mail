...
| Untracked source accounts | 🔵 built | `untracked:<key>` in `mapping.json` (value `null`) says a source account is deliberately outside the budget. Its rows are set aside before the reconciliation floor, before transfer pairing and before any FX fetch, counted on the run line, and the keys in force are named on stdout. One entry per non-base balance. A near-miss prefix, a non-null value or a key over a pot target stops the run, because an inert licence writes the money it should have set aside. Wise holds a balance per currency while the budget holds one Wise account, so without this a `wise-aud` row was FX-converted into it and booked as an SGD debit that never happened |
