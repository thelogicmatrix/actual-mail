...
| `SOURCE FAILED wise: Wise unreachable after 6 attempts` | Your resolver returned EAI_AGAIN, or Wise is genuinely down | Nothing to do. Six attempts over ~31s already rode the blip out, the Trust rows in that run were still written, and the next hourly pass re-covers the window. Only investigate if it repeats across several runs |
| `SOURCE FAILED trust-sg: …` | IMAP unreachable, or refusing the App Password | Same shape: the Wise rows still landed. Since 2026-08-03 `connect()` also retries a transient code, 6× over ~31s since 2026-09-25, so a blip should no longer reach this line. A credential failure is NOT retried and shows up on the first attempt. Check the App Password if it persists past one run |
