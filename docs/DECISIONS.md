...
- The licence **beats** an ordinary key rather than replacing it, so a leftover `wise-aud` entry is inert rather than contradictory. No warning for that case: unlike an orphan `no-inbound-alert:` licence, which is silently inert and loses a link, a redundant key here changes no behaviour at all.
- Status: built, not yet run against live mail. Six tests, in `test/load/load.test.js` and `test/load-cli.test.js`.
