...
- The licence **beats** an ordinary key rather than replacing it, so a leftover `wise-aud` entry is inert rather than contradictory. That was not true when first written: the second pairing pass at `src/load/load.js` took the unsplit `rows`, so an untracked row could be a live pairing candidate whenever a leftover ordinary key made it resolvable, and it then contested a licensed row and silently downgraded a real internal transfer to an ordinary spend — no `ambiguous`, no `left unlinked`, nothing on the run line. Both passes now take `tracked`. The second pass still sees rows the reconciliation floor skipped, which is what it is for; `splitUntracked` runs before `inScope`, so that is unchanged. Pinned by a test that runs the mapping with and without the leftover key and compares the counts.
- **Every non-base balance needs its own entry, and the live mapping had two.** `wise-aud` and `wise-usd` both resolved to the one SGD Wise account. Documenting only `wise-aud` would have had an operator fix AUD and leave USD armed to do the same thing on its first movement, which had not happened yet only because no USD movement had occurred.
- A **typo is refused, not absorbed.** A near-miss prefix is worse than a missing key: `untraked:wise-aud` is not a licence, so the ordinary key beside it still resolves and the row is FX-converted into the wrong account while the run reports success — the exact defect this feature exists to stop, reintroduced by one character. There is no fuzzy match for a typo and none is needed: the mapping has exactly three legal prefixes, so any other prefixed key stops the run. Refusing beats warning, because a warning still writes the money. Also refused: an `untracked:` value that is not `null` (it reaches the `Object.values(mapping)` dedupe scan and makes the loader read an account that need not exist), and an `untracked:` key over a pot target (it matches a row's source account, so it could never have fired).
...
- A **pot move out of an untracked account is a hard error**, not a row set aside. The pot is inside the budget, so the arrival really happened and dropping it is the quiet loss this loader exists to prevent. Unreachable on today's mapping, where the untracked accounts are foreign-currency balances and the pots belong to one bank, which is why it is pinned rather than assumed.
- Rejected: **warning every run about an `untracked:` key that matched no row.** A quiet week for one account is normal and the warning would fire constantly, which trains the operator to ignore the channel that carries real faults. The stdout listing gives the same information once, on demand, without the noise.
- Status: built, not yet run against live mail. Fifteen tests, in `test/load/load.test.js`, `test/load-cli.test.js` and `test/scan-pii.test.js`.
