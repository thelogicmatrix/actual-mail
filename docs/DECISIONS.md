...
- Refused, falling back to the old reporting: a **reconciled** existing row (the human has balanced that period, and deleting unbalances it) and a row **already part of a transfer** (it is not stale). Both err toward reporting, because the cost of reporting is a missing link and the cost of deleting the wrong row is money.
- Deleting is an **optional api capability**. `scripts/verify-scratch.js` and any external caller pass a two-method api, so relinking degrades to the old behaviour rather than throwing on a missing method.
...
- The dry-run sink carries `deleteTransaction` and reports `WOULD DELETE` instead of deleting. Dropping the method instead would make a rehearsal take a different code path from the real run, which is the one thing a rehearsal exists to rule out.
...
- Status: built 2026-08-28. Five unit tests and two CLI tests, including one that proves a dry run deletes nothing.
